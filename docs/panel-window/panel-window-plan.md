...
- **Фаза 5** (owner 2026-07-14) — **второй потребитель контрола: «Список треков»** (переим. из
  «Голоса расписания»). Slide-over `<aside>` в `TracksPanel` → `PanelWindow`. **Размещение —
  внутри TracksPanel** (не в AudioPage, как file-open), т.к. `gtracks = useGtrackLanes(...)` —
  per-call фабрика в setup TracksPanel, не singleton-стор: хостинг там даёт floating+docked общее
  реактивное состояние голосов без прокидки (PW-D9). Кнопка панели — крайняя левая перед Zoom+ и
  **всегда видима** (снят гейт `displayMode==='gnaural'`, в т.ч. для wav/flac). В список
  ...
  «речь идёт только о СПИСКЕ треков, а не о графике»). **Detached — отдельный шаг/owner-чекпоинт**
  (PW5.5): дочернее окно — отдельный экземпляр приложения без загруженного расписания/анализа →
  список голосов там пуст, пока не прокинуть снимок или не переанализировать файл. Шаги PW5.1..PW5.5.
