...
- **Фаза 5** (owner 2026-07-14) — **второй потребитель контрола: «Список треков»** (переим. из
  ...
  **всегда видима** (снят гейт `displayMode==='gnaural'`, в т.ч. для wav/flac). В список
  ...
  пункт меню скрыт через `allowDetach=false`. Настоящий detach оформлен как **PW5.7 — «пульт»**:
  ребёнок без своего состояния рендерит снимок от родителя и шлёт каждое действие по мосту назад
  (родительский gtracks — единственный источник правды). **PW5.8** (owner 2026-07-14): полное
