...
- **Фаза 5** (owner 2026-07-14) — **второй потребитель контрола: «Список треков»** (переим. из
  «Голоса расписания»). Slide-over `<aside>` в `TracksPanel` → `PanelWindow`. **Размещение:** сперва внутри
  TracksPanel (PW-D9), затем по уточнению владельца перенесено на **уровень AudioPage** — общий
  singleton `useSharedGtrackLanes()`, панель дочится на **всю высоту редактора** в том же доке,
  что и «Открыть файл», и переживает переключение вкладок (PW5.6a–c, PW-D10). Кнопка панели — крайняя левая перед Zoom+ и
  **всегда видима** (снят гейт `displayMode==='gnaural'`, в т.ч. для wav/flac). В список
  добавляются строки **«Общая волна»/«Общий спектр»** (имя + видимость; переиспользуются ключи
  `tracksOverallWave/tracksOverallSpectrum`) — только строки, **не графики** (уточнение владельца:
  «речь идёт только о СПИСКЕ треков, а не о графике»). **Detached:** наивный вариант (PW5.5)
  заблокирован — попап живёт в том же процессе (PW-D1), но в своём JS-мире: свой Vue/Pinia, без
  загруженного расписания, а собственный gtracks затирал бы общие localStorage-ключи (PW-D11);
  пункт меню временно скрыт через `allowDetach=false`. Настоящий detach реализован как **PW5.7 —
  «пульт»** (a–c, done 2026-07-14): ребёнок (`TrackListRemote`) без своего состояния рендерит снимок
  от родителя (`:bridge-state`) и шлёт каждое действие по мосту назад одним сериализуемым `action`
  (родительский gtracks — единственный источник правды; общий `applyTrackListAction` на обеих
  сторонах); `allowDetach=false` снят, `TrackListView`/`TrackListPanel` — чистое view + in-window
  адаптер. **PW5.8** (owner 2026-07-14): полное
  скрытие общей волны/спектра (с заголовком), как у gtrack-дорожек — Ctrl-click по контролу
  заголовка + глаз в списке скрывают полностью (норм. клик = свернуть), восстановление через
  «Скрытые дорожки». Шаги PW5.1..PW5.8.
