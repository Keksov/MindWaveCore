...
- [ ] **PR2.5 — PAUSE: owner проверяет «редактор всё вспоминает» после перезапуска.**
...
- [ ] **PR4.2 — Удаление.** `deleteProject` (с защитой «только внутри `projects/`»), ручной GC
  осиротевших из списка. Verify: `bun test project-store` + вручную.
- [ ] **PR4.3 — UI «Проекты».** Простая панель в настройках: список, статус, удалить / re-link /
