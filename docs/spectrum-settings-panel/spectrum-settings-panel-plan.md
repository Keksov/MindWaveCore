...
- **SS3.2** — `SpectrogramSettingsRemote.vue` + запись в `panels[]` манифеста
  ...
  `allow-detach=false`. Пресеты в ребёнке — «Применить» + «Сохранить как…»; «Управление…» скрыто
  пропом `allowPresetManage=false` (SS-D8 rev., req 7) — экспорт/импорт через мост не требуется.
