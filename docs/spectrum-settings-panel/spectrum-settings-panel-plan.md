...
- **SS-D5 — аккордеон = `q-expansion-item`** (подтверждён req 6) по образцу
  ...
  (`mindwave-panel-spectrum-settings-groups`). По умолчанию открыт первый блок («Пресеты», SS-D6);
  если на SS2.2 это окажется неудобным (блок в одну строку), дефолт — одна строка правки.
- **SS-D6 — REVISED (owner req 5) — пять блоков: «Пресеты» + четыре группы; «Сброс» — сам по себе
  в футере.** Ответ владельца **гибридный**, и обе его половины важны по отдельности:
