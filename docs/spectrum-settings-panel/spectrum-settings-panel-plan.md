...
- **SS2.7** (owner 2026-07-16, скриншот, после SS2.6) — регулярная сетка с равновысокими строками
  ...
  высоте, без пустот. Рамки/flush из SS2.5/2.6 сохранены; межблочные линии в masonry рваные по
  высоте (неизбежно). Важно: multicol игнорируется на `display:flex`, поэтому `--tiles` ставит
  `display:block`.
